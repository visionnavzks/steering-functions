...
namespace steering
{
    ...
    CC_Dubins_State_Space::CC_Dubins_State_Space(double kappa,
                                                 double sigma,
    ...
    vector<pair<State, Control>> CC_Dubins_State_Space::predict_state(const State& state,
                                                                      bool         forwards) const
    {
        ...
        for (auto& state_control : states_controls)
        {
            ...
            end_of_clothoid(state.x,
                            ...
                            d,
                            abs_delta_s,
                            &state_control.first.x,
                            &state_control.first.y,
                            &state_control.first.theta,
                            &state_control.first.kappa);
        }
        return states_controls;
    }
                ...
                else
                {
                    ...
                    if (fabs(start_state.kappa) < get_epsilon())
                    {
                        if (fabs(end_state.kappa) < get_epsilon())
                            distance +=
                                cc00_dubins_state_space_.get_distance(start_state, end_state);
                        else
                            distance +=
                                cc0pm_dubins_state_space_.get_distance(start_state, end_state);
                    }
                    else
                    {
                        if (fabs(end_state.kappa) < get_epsilon())
                            distance +=
                                ccpm0_dubins_state_space_.get_distance(start_state, end_state);
                        else
                            distance +=
                                ccpmpm_dubins_state_space_.get_distance(start_state, end_state);
                    }
    ...
    vector<Control> CC_Dubins_State_Space::get_controls(const State& state1,
                                                        const State& state2) const
    {
            ...
            for (const auto& end_state_control : end_states_controls)
            {
                ...
                else
                {
                    if (fabs(start_state.kappa) < get_epsilon())
                    {
                        if (fabs(end_state.kappa) < get_epsilon())
                            cc_dubins_controls =
                                cc00_dubins_state_space_.get_controls(start_state, end_state);
                        else
                            cc_dubins_controls =
                                cc0pm_dubins_state_space_.get_controls(start_state, end_state);
                    }
                    else
                    {
                        if (fabs(end_state.kappa) < get_epsilon())
                            cc_dubins_controls =
                                ccpm0_dubins_state_space_.get_controls(start_state, end_state);
                        else
                            cc_dubins_controls =
                                ccpmpm_dubins_state_space_.get_controls(start_state, end_state);
                    }
                    // adjust controls by intial and final control
                    if (fabs(start_control.delta_s) > get_epsilon())
                    {
        ...
        sort(cc_dubins_controls_distance_pairs.begin(),
             cc_dubins_controls_distance_pairs.end(),
    ...
    vector<Control> CC_Dubins_State_Space::get_controls_reverse(const State& state1,
                                                               const State& state2) const
    {
    ...
    vector<vector<Control>> CC_Dubins_State_Space::get_all_controls(const State& state1,
                                                                   const State& state2) const
    {
        vector<vector<Control>> all_controls;
        // Add forward driving path
        all_controls.emplace_back(get_controls(state1, state2));
        // Add reverse driving path
        all_controls.emplace_back(get_controls_reverse(state1, state2));
        return all_controls;
    }
