...
namespace steering
{
    ...
    Matrix3d EKF::covariance_to_eigen(const double covariance[16]) const
    {
        ...
        for (int i = 0, n = covariance_eigen.rows(); i < n; ++i)
        {
            ...
            {
                covariance_eigen(i, j) =
                    covariance[(i << 2) + j]; // bitwise operation for: covariance[i * 4 + j]
            }
        }
    ...
    void EKF::eigen_to_covariance(const Matrix3d& covariance_eigen, double covariance[16]) const
    {
        for (int i = 0, n = covariance_eigen.rows(); i < n; ++i)
        {
            ...
            {
                covariance[(i << 2) + j] =
                    covariance_eigen(i, j); // bitwise operation for: covariance[i * 4 + j]
            }
        }
    }

    void EKF::get_motion_jacobi(const State&   state,
                                ...
                                Matrix32d&     F_u) const
    {
        ...
        if (fabs(control.sigma) > get_epsilon())
        {
            ...
            double k2 = SQRT_PI_INV * sqrt_sigma_inv *
                        (abs_sigma * integration_step + sgn_sigma * state.kappa);
        ...
        else
        {
            if (fabs(state.kappa) > get_epsilon())
            {
                ...
                F_u(0, 1) = (sin_th - sin_k9) / (state.kappa * state.kappa) +
                            d * integration_step * cos_k9 / state.kappa;
                F_u(1, 1) = (-cos_th + cos_k9) / (state.kappa * state.kappa) +
    ...
    Matrix2d EKF::get_motion_covariance(const State&   state,
                                        const Control& control,
                                        double         integration_step) const
    {
        Matrix2d Q(Matrix2d::Zero());
        Q(0, 0) = motion_noise_.alpha1 * integration_step * integration_step +
                  motion_noise_.alpha2 * state.kappa * state.kappa;
        Q(1, 1) = motion_noise_.alpha3 * integration_step * integration_step +
                  motion_noise_.alpha4 * state.kappa * state.kappa;
        return Q;
    }
    ...
    void EKF::predict(const State_With_Covariance& state,
                      const Control&               control,
                      double                       integration_step,
    ...
    void EKF::update(const State_With_Covariance& state_pred,
