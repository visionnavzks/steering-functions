...
namespace steering
{
    ...
    HC_Reeds_Shepp_State_Space::HC_Reeds_Shepp_State_Space(double kappa,
                                                           double sigma,
    ...
    vector<pair<State, Control>> HC_Reeds_Shepp_State_Space::predict_state(const State& state) const
    {
        ...
        if ((fabs(state.kappa) < get_epsilon()) ||
            (fabs(kappa_ - fabs(state.kappa)) < get_epsilon()))
        {
        ...
        if (fabs(state.kappa) > kappa_)
        { // if kappa is greater than kappa_max, then sigma is negative
            state_control1.second.sigma = -sgn_kappa * sigma_;
        }
                ...
                else
                {
                    ...
                    if (fabs(start_state.kappa) < get_epsilon())
                    {
                        if (fabs(end_state.kappa) < get_epsilon())
                            distance +=
                                hc00_reeds_shepp_state_space_.get_distance(start_state, end_state);
                        else
                            distance +=
                                hc0pm_reeds_shepp_state_space_.get_distance(start_state, end_state);
                    }
                    else
                    {
                        if (fabs(end_state.kappa) < get_epsilon())
                            distance +=
                                hcpm0_reeds_shepp_state_space_.get_distance(start_state, end_state);
                        else
                            distance += hcpmpm_reeds_shepp_state_space_.get_distance(start_state,
                                                                                     end_state);
                    }
    ...
    vector<Control> HC_Reeds_Shepp_State_Space::get_controls(const State& state1,
                                                             const State& state2) const
    {
            ...
            for (const auto& end_state_control : end_states_controls)
            {
                ...
                else
                {
                    if (fabs(start_state.kappa) < get_epsilon())
                    {
                        if (fabs(end_state.kappa) < get_epsilon())
                            hc_rs_controls =
                                hc00_reeds_shepp_state_space_.get_controls(start_state, end_state);
                        else
                            hc_rs_controls =
                                hc0pm_reeds_shepp_state_space_.get_controls(start_state, end_state);
                    }
                    else
                    {
                        if (fabs(end_state.kappa) < get_epsilon())
                            hc_rs_controls =
                                hcpm0_reeds_shepp_state_space_.get_controls(start_state, end_state);
                        else
                            hc_rs_controls = hcpmpm_reeds_shepp_state_space_.get_controls(
        ...
        sort(hc_rs_controls_distance_pairs.begin(),
             hc_rs_controls_distance_pairs.end(),
