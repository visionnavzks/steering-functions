...
namespace steering
{
    ...
    public:
        explicit HCpmpm_Reeds_Shepp(HCpmpm_Reeds_Shepp_State_Space* parent) { parent_ = parent; }
        ...
        {
            return path_helpers::TT_exists(distance, c1, c2, parent_->radius_);
        }
        void
        TT_tangent_circles(const HC_CC_Circle& c1, const HC_CC_Circle& c2, Configuration** q) const
        {
            path_helpers::TT_tangent_circles(c1, c2, parent_->mu_, q);
        }
                       ...
                       Configuration**     q3) const
        {
            ...
            *cstart = new HC_CC_Circle(
                **q2, c1.left, !c1.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            *cend = new HC_CC_Circle(
                **q2, c2.left, !c2.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
        ...
        void
        TcT_tangent_circles(const HC_CC_Circle& c1, const HC_CC_Circle& c2, Configuration** q) const
        {
        ...
        double TcT_path(const HC_CC_Circle& c1,
                        const HC_CC_Circle& c2,
                        HC_CC_Circle**      cstart,
                        HC_CC_Circle**      cend,
        ...
        double TcTcT_path(const HC_CC_Circle& c1,
                          const HC_CC_Circle& c2,
                          HC_CC_Circle**      cstart,
                          HC_CC_Circle**      cend,
                          Configuration**     q1,
                          ...
                          HC_CC_Circle**      ci) const
        {
            ...
            *cend   = new HC_CC_Circle(c2);
            ...
            double length1 = (*cstart)->rs_turn_length(*qa) + middle1->rs_turn_length(*qb) +
                             (*cend)->rs_turn_length(*qb);
            double length2 = (*cstart)->rs_turn_length(*qc) + middle2->rs_turn_length(*qd) +
        ...
        {
            return path_helpers::TcTT_exists(distance, c1, c2, 2 * parent_->radius_ + 2 * fabs(c1.kappa_inv), 2 * parent_->radius_ - 2 * fabs(c1.kappa_inv));
        }
                                  ...
                                  Configuration**     q4) const
        {
            ...
            global_frame_change(c1.xc, c1.yc, theta, delta_x, delta_y, &x, &y);
            HC_CC_Circle tgt1(
                x, y, !c1.left, !c1.forward, c1.regular, parent_->hc_cc_circle_param_);
            global_frame_change(c1.xc, c1.yc, theta, delta_x, -delta_y, &x, &y);
            HC_CC_Circle tgt2(
                x, y, !c1.left, !c1.forward, c1.regular, parent_->hc_cc_circle_param_);
        ...
        double TcTT_path(const HC_CC_Circle& c1,
                         const HC_CC_Circle& c2,
                         HC_CC_Circle**      cstart,
                         HC_CC_Circle**      cend,
                         Configuration**     q1,
                         ...
                         HC_CC_Circle**      ci) const
        {
            ...
            end1 = new HC_CC_Circle(
                *qb, c2.left, !c2.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            end2 = new HC_CC_Circle(
                *qd, c2.left, !c2.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            middle1 =
                new HC_CC_Circle(*qb, !c1.left, c1.forward, true, parent_->hc_cc_circle_param_);
            middle2 =
                new HC_CC_Circle(*qd, !c1.left, c1.forward, true, parent_->hc_cc_circle_param_);

            *cstart = new HC_CC_Circle(c1);
            *q2     = new Configuration(c2.start.x, c2.start.y, c2.start.theta, c2.kappa);

            // select shortest connection
            double length1 = (*cstart)->rs_turn_length(*qa) + middle1->hc_turn_length(*qa) +
                             end1->hc_turn_length(**q2);
            double length2 = (*cstart)->rs_turn_length(*qc) + middle2->hc_turn_length(*qc) +
        ...
        {
            return path_helpers::TTcT_exists(distance, c1, c2, 2 * parent_->radius_ + 2 * fabs(c1.kappa_inv), 2 * parent_->radius_ - 2 * fabs(c1.kappa_inv));
        }
        ...
        double TTcT_path(const HC_CC_Circle& c1,
                         const HC_CC_Circle& c2,
                         HC_CC_Circle**      cstart,
                         HC_CC_Circle**      cend,
                         Configuration**     q1,
                         ...
                         HC_CC_Circle**      ci) const
        {
            ...
            start1 = new HC_CC_Circle(
                *qa, c1.left, !c1.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            start2 = new HC_CC_Circle(
                *qc, c1.left, !c1.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            middle1 =
                new HC_CC_Circle(*qa, !c1.left, c1.forward, true, parent_->hc_cc_circle_param_);
            middle2 =
                new HC_CC_Circle(*qc, !c1.left, c1.forward, true, parent_->hc_cc_circle_param_);

            *cend = new HC_CC_Circle(c2);
            *q1   = new Configuration(c1.start.x, c1.start.y, c1.start.theta, c1.kappa);

            // select shortest connection
            double length1 = start1->hc_turn_length(**q1) + middle1->hc_turn_length(*qb) +
                             (*cend)->rs_turn_length(*qb);
            double length2 = start2->hc_turn_length(**q1) + middle2->hc_turn_length(*qd) +
                             (*cend)->rs_turn_length(*qd);
            if (length1 < length2)
            {
                *cstart = start1;
        ...
        {
            return path_helpers::TiST_exists(distance, c1, c2, parent_->radius_);
        }
        bool TeST_exists(const HC_CC_Circle& c1, const HC_CC_Circle& c2) const
        {
            return path_helpers::TeST_exists(distance, c1, c2, parent_->radius_ * parent_->sin_mu_);
        }
        bool TST_exists(const HC_CC_Circle& c1, const HC_CC_Circle& c2) const
        {
            return path_helpers::TST_exists(distance, c1, c2, parent_->radius_, parent_->radius_ * parent_->sin_mu_);
        }
        ...
        {
            path_helpers::TiST_tangent_circles(c1, c2, parent_->radius_, parent_->sin_mu_, parent_->cos_mu_, q1, q2);
        }
        void TeST_tangent_circles(const HC_CC_Circle& c1,
                                  ...
                                  Configuration**     q2) const
        {
            path_helpers::TeST_tangent_circles(c1, c2, parent_->radius_, parent_->sin_mu_, parent_->cos_mu_, q1, q2);
        }
                         ...
                         Configuration**     q4) const
        {
            ...
            *cstart = new HC_CC_Circle(
                **q2, c1.left, !c1.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            *cend = new HC_CC_Circle(
                **q3, c2.left, !c2.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            *q1 = new Configuration(c1.start.x, c1.start.y, c1.start.theta, c1.kappa);
            *q4 = new Configuration(c2.start.x, c2.start.y, c2.start.theta, c2.kappa);
            return (*cstart)->hc_turn_length(**q1) + configuration_distance(**q2, **q3) +
                   (*cend)->hc_turn_length(**q4);
        }

        double TeST_path(const HC_CC_Circle& c1,
                         const HC_CC_Circle& c2,
                         HC_CC_Circle**      cstart,
                         HC_CC_Circle**      cend,
                         Configuration**     q1,
        ...
        double TST_path(const HC_CC_Circle& c1,
                        const HC_CC_Circle& c2,
                        HC_CC_Circle**      cstart,
                        HC_CC_Circle**      cend,
                        Configuration**     q1,
        ...
        bool TiSTcT_exists(const HC_CC_Circle& c1, const HC_CC_Circle& c2) const
        {
            ...
            return distance >=
                   sqrt(pow(2 * parent_->radius_ * parent_->sin_mu_ + 2 * fabs(c1.kappa_inv), 2) +
        ...
        double TiSTcT_path(const HC_CC_Circle& c1,
                           const HC_CC_Circle& c2,
                           HC_CC_Circle**      cstart,
                           HC_CC_Circle**      cend,
                           Configuration**     q1,
                           ...
                           HC_CC_Circle**      ci) const
        {
            double theta = angle;
            double delta_y =
                (4 * parent_->radius_ * parent_->cos_mu_) / (fabs(c2.kappa) * distance);
        ...
        double TeSTcT_path(const HC_CC_Circle& c1,
                           const HC_CC_Circle& c2,
                           HC_CC_Circle**      cstart,
                           HC_CC_Circle**      cend,
                           Configuration**     q1,
        ...
        double TSTcT_path(const HC_CC_Circle& c1,
                          const HC_CC_Circle& c2,
                          HC_CC_Circle**      cstart,
                          HC_CC_Circle**      cend,
                          Configuration**     q1,
        ...
        double TcTiST_path(const HC_CC_Circle& c1,
                           const HC_CC_Circle& c2,
                           HC_CC_Circle**      cstart,
                           HC_CC_Circle**      cend,
                           Configuration**     q1,
                           ...
                           HC_CC_Circle**      ci) const
        {
            ...
            global_frame_change(c1.xc, c1.yc, theta, delta_x, -delta_y, &x, &y);
            HC_CC_Circle tgt1(
                x, y, !c2.left, !c2.forward, c2.regular, parent_->hc_cc_circle_param_);
        ...
        double TcTeST_path(const HC_CC_Circle& c1,
                           const HC_CC_Circle& c2,
                           HC_CC_Circle**      cstart,
                           HC_CC_Circle**      cend,
                           Configuration**     q1,
        ...
        double TcTST_path(const HC_CC_Circle& c1,
                          const HC_CC_Circle& c2,
                          HC_CC_Circle**      cstart,
                          HC_CC_Circle**      cend,
                          Configuration**     q1,
        ...
        bool TcTiSTcT_exists(const HC_CC_Circle& c1, const HC_CC_Circle& c2) const
        {
            ...
            return (distance >= sqrt(pow(2 * parent_->radius_, 2) +
                                     16 * parent_->radius_ * parent_->sin_mu_ * fabs(c1.kappa_inv) +
        ...
        double TcTiSTcT_path(const HC_CC_Circle& c1,
                             const HC_CC_Circle& c2,
                             HC_CC_Circle**      cstart,
                             HC_CC_Circle**      cend,
                             Configuration**     q1,
                             ...
                             HC_CC_Circle**      ci2) const
        {
            double theta = angle;
            double delta_y =
                (4 * parent_->radius_ * parent_->cos_mu_) / (distance * fabs(c1.kappa));
        ...
        double TcTeSTcT_path(const HC_CC_Circle& c1,
                             const HC_CC_Circle& c2,
                             HC_CC_Circle**      cstart,
                             HC_CC_Circle**      cend,
                             Configuration**     q1,
        ...
        double TcTSTcT_path(const HC_CC_Circle& c1,
                            const HC_CC_Circle& c2,
                            HC_CC_Circle**      cstart,
                            HC_CC_Circle**      cend,
                            Configuration**     q1,
        ...
        {
            return path_helpers::TTcTT_exists(distance, c1, c2, 4 * parent_->radius_ + 2 * fabs(c1.kappa_inv));
        }
        ...
        double TTcTT_path(const HC_CC_Circle& c1,
                          const HC_CC_Circle& c2,
                          HC_CC_Circle**      cstart,
                          HC_CC_Circle**      cend,
                          Configuration**     q1,
                          ...
                          HC_CC_Circle**      ci2) const
        {
            ...
            middle1 =
                new HC_CC_Circle(*qa, !c1.left, c1.forward, true, parent_->hc_cc_circle_param_);
            middle2 =
                new HC_CC_Circle(*qc, !c2.left, c2.forward, true, parent_->hc_cc_circle_param_);
            end1 = new HC_CC_Circle(
                *qc, c2.left, !c2.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            start2 = new HC_CC_Circle(
                *qd, c1.left, !c1.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            middle3 =
                new HC_CC_Circle(*qd, !c1.left, c1.forward, true, parent_->hc_cc_circle_param_);
            middle4 =
                new HC_CC_Circle(*qf, !c2.left, c2.forward, true, parent_->hc_cc_circle_param_);
            end2 = new HC_CC_Circle(
                *qf, c2.left, !c2.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            ...
            *q3 = new Configuration(c2.start.x, c2.start.y, c2.start.theta, c2.kappa);
            ...
            double length2 = start2->hc_turn_length(**q1) + middle3->hc_turn_length(*qe) +
        ...
        {
            return path_helpers::TcTTcT_exists(distance, c1, c2, 4 * fabs(c1.kappa_inv) + 2 * parent_->radius_, 4 * fabs(c1.kappa_inv) - 2 * parent_->radius_);
        }
        ...
        double TcTTcT_path(const HC_CC_Circle& c1,
                           const HC_CC_Circle& c2,
                           HC_CC_Circle**      cstart,
                           HC_CC_Circle**      cend,
                           Configuration**     q1,
                           ...
                           HC_CC_Circle**      ci2) const
        {
            ...
            middle2 =
                new HC_CC_Circle(*qb, c1.left, !c1.forward, true, parent_->hc_cc_circle_param_);
            middle3 =
                new HC_CC_Circle(*qe, !c1.left, c1.forward, true, parent_->hc_cc_circle_param_);
            middle4 =
                new HC_CC_Circle(*qe, c1.left, !c1.forward, true, parent_->hc_cc_circle_param_);
            ...
            *cend   = new HC_CC_Circle(c2);
            ...
            double length2 = (*cstart)->rs_turn_length(*qd) + middle3->hc_turn_length(*qd) +
        ...
        {
            return path_helpers::TTT_exists(distance, c1, c2, parent_->radius_);
        }
        ...
        double TTT_path(const HC_CC_Circle& c1,
                        const HC_CC_Circle& c2,
                        HC_CC_Circle**      cstart,
                        HC_CC_Circle**      cend,
                        Configuration**     q1,
                        ...
                        HC_CC_Circle**      ci) const
        {
            ...
            start1 = new HC_CC_Circle(
                *qa, c1.left, !c1.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            middle1 = new HC_CC_Circle(
                *qa, !c1.left, c1.forward, CC_REGULAR, parent_->hc_cc_circle_param_);
            end1 = new HC_CC_Circle(
                *qb, c2.left, !c2.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            start2 = new HC_CC_Circle(
                *qc, c1.left, !c1.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            middle2 = new HC_CC_Circle(
                *qc, !c1.left, c1.forward, CC_REGULAR, parent_->hc_cc_circle_param_);
            end2 = new HC_CC_Circle(
                *qd, c2.left, !c2.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            ...
            *q3 = new Configuration(c2.start.x, c2.start.y, c2.start.theta, c2.kappa);
            ...
            // select shortest connection
            double length1 = start1->hc_turn_length(**q1) + middle1->cc_turn_length(*qb) +
                             end1->hc_turn_length(**q3);
            double length2 = start2->hc_turn_length(**q1) + middle2->cc_turn_length(*qd) +
        ...
        bool TciST_exists(const HC_CC_Circle& c1, const HC_CC_Circle& c2) const
        {
            ...
            return distance >=
                   sqrt(pow(parent_->radius_ * parent_->sin_mu_, 2) +
        ...
        double TciST_path(const HC_CC_Circle& c1,
                          const HC_CC_Circle& c2,
                          HC_CC_Circle**      cstart,
                          HC_CC_Circle**      cend,
                          Configuration**     q1,
        ...
        {
            double alpha =
                asin((parent_->radius_ * parent_->cos_mu_ + fabs(c1.kappa_inv)) / distance);
            double delta_x1 = 0.0;
            ...
            *cend   = new HC_CC_Circle(
                **q2, c2.left, !c2.forward, HC_REGULAR, parent_->hc_cc_circle_param_);
            return (*cstart)->rs_turn_length(**q1) + configuration_distance(**q1, **q2) +
                   (*cend)->hc_turn_length(**q3);
        }

        double TceST_path(const HC_CC_Circle& c1,
                          const HC_CC_Circle& c2,
                          HC_CC_Circle**      cstart,
                          HC_CC_Circle**      cend,
                          Configuration**     q1,
        ...
        double TcST_path(const HC_CC_Circle& c1,
                         const HC_CC_Circle& c2,
                         HC_CC_Circle**      cstart,
                         HC_CC_Circle**      cend,
                         Configuration**     q1,
        ...
        double TiScT_path(const HC_CC_Circle& c1,
                          const HC_CC_Circle& c2,
                          HC_CC_Circle**      cstart,
                          HC_CC_Circle**      cend,
                          Configuration**     q1,
        ...
        double TeScT_path(const HC_CC_Circle& c1,
                          const HC_CC_Circle& c2,
                          HC_CC_Circle**      cstart,
                          HC_CC_Circle**      cend,
                          Configuration**     q1,
        ...
        double TScT_path(const HC_CC_Circle& c1,
                         const HC_CC_Circle& c2,
                         HC_CC_Circle**      cstart,
                         HC_CC_Circle**      cend,
                         Configuration**     q1,
        ...
        double TciScT_path(const HC_CC_Circle& c1,
                           const HC_CC_Circle& c2,
                           HC_CC_Circle**      cstart,
                           HC_CC_Circle**      cend,
                           Configuration**     q1,
        ...
        double TceScT_path(const HC_CC_Circle& c1,
                           const HC_CC_Circle& c2,
                           HC_CC_Circle**      cstart,
                           HC_CC_Circle**      cend,
                           Configuration**     q1,
        ...
        double TcScT_path(const HC_CC_Circle& c1,
                          const HC_CC_Circle& c2,
                          HC_CC_Circle**      cstart,
                          HC_CC_Circle**      cend,
                          Configuration**     q1,
    ...
    HCpmpm_Reeds_Shepp_State_Space::HCpmpm_Reeds_Shepp_State_Space(double kappa,
                                                                   double sigma,
    ...
    {
        rs_circle_param_.set_param(
            kappa_, numeric_limits<double>::max(), 1 / kappa_, 0.0, 0.0, 1.0, 0.0);
    ...
    HC_CC_RS_Path*
    HCpmpm_Reeds_Shepp_State_Space::hcpmpm_circles_rs_path(const HC_CC_Circle& c1,
                                                           const HC_CC_Circle& c2) const
    {
        ...
        {
            cstart[hc_cc_rs::T] =
                new HC_CC_Circle(c1.start, c1.left, c1.forward, true, rs_circle_param_);
        ...
        {
            length[hc_cc_rs::TT] = hcpmpm_reeds_shepp_->TT_path(c1,
                                                                c2,
                                                                &cstart[hc_cc_rs::TT],
                                                                &cend[hc_cc_rs::TT],
                                                                &qi1[hc_cc_rs::TT],
        ...
        {
            length[hc_cc_rs::TcT] = hcpmpm_reeds_shepp_->TcT_path(
                c1, c2, &cstart[hc_cc_rs::TcT], &cend[hc_cc_rs::TcT], &qi1[hc_cc_rs::TcT]);
        }
        // ##### Reeds-Shepp families: ############################################
        // case TcTcT
        if (hcpmpm_reeds_shepp_->TcTcT_exists(c1, c2))
        {
            length[hc_cc_rs::TcTcT] = hcpmpm_reeds_shepp_->TcTcT_path(c1,
                                                                      c2,
                                                                      &cstart[hc_cc_rs::TcTcT],
                                                                      &cend[hc_cc_rs::TcTcT],
                                                                      &qi1[hc_cc_rs::TcTcT],
                                                                      &qi2[hc_cc_rs::TcTcT],
                                                                      &ci1[hc_cc_rs::TcTcT]);
        ...
        {
            length[hc_cc_rs::TcTT] = hcpmpm_reeds_shepp_->TcTT_path(c1,
                                                                    c2,
                                                                    &cstart[hc_cc_rs::TcTT],
                                                                    &cend[hc_cc_rs::TcTT],
                                                                    &qi1[hc_cc_rs::TcTT],
                                                                    &qi2[hc_cc_rs::TcTT],
                                                                    &ci1[hc_cc_rs::TcTT]);
        ...
        {
            length[hc_cc_rs::TTcT] = hcpmpm_reeds_shepp_->TTcT_path(c1,
                                                                    c2,
                                                                    &cstart[hc_cc_rs::TTcT],
                                                                    &cend[hc_cc_rs::TTcT],
                                                                    &qi1[hc_cc_rs::TTcT],
                                                                    &qi2[hc_cc_rs::TTcT],
                                                                    &ci1[hc_cc_rs::TTcT]);
        ...
        {
            length[hc_cc_rs::TST] = hcpmpm_reeds_shepp_->TST_path(c1,
                                                                  c2,
                                                                  &cstart[hc_cc_rs::TST],
                                                                  &cend[hc_cc_rs::TST],
                                                                  &qi1[hc_cc_rs::TST],
        ...
        {
            length[hc_cc_rs::TSTcT] = hcpmpm_reeds_shepp_->TSTcT_path(c1,
                                                                      c2,
                                                                      &cstart[hc_cc_rs::TSTcT],
                                                                      &cend[hc_cc_rs::TSTcT],
                                                                      &qi1[hc_cc_rs::TSTcT],
                                                                      &qi2[hc_cc_rs::TSTcT],
                                                                      &qi3[hc_cc_rs::TSTcT],
        ...
        {
            length[hc_cc_rs::TcTST] = hcpmpm_reeds_shepp_->TcTST_path(c1,
                                                                      c2,
                                                                      &cstart[hc_cc_rs::TcTST],
                                                                      &cend[hc_cc_rs::TcTST],
                                                                      &qi1[hc_cc_rs::TcTST],
                                                                      &qi2[hc_cc_rs::TcTST],
                                                                      &qi3[hc_cc_rs::TcTST],
        ...
        if (hcpmpm_reeds_shepp_->TcTSTcT_exists(c1, c2))
        {
            length[hc_cc_rs::TcTSTcT] =
                hcpmpm_reeds_shepp_->TcTSTcT_path(c1,
                                                  c2,
                                                  &cstart[hc_cc_rs::TcTSTcT],
                                                  &cend[hc_cc_rs::TcTSTcT],
                                                  &qi1[hc_cc_rs::TcTSTcT],
                                                  &qi2[hc_cc_rs::TcTSTcT],
                                                  &qi3[hc_cc_rs::TcTSTcT],
                                                  &qi4[hc_cc_rs::TcTSTcT],
                                                  &ci1[hc_cc_rs::TcTSTcT],
                                                  &ci2[hc_cc_rs::TcTSTcT]);
        }
        // case TTcTT
        if (hcpmpm_reeds_shepp_->TTcTT_exists(c1, c2))
        {
            length[hc_cc_rs::TTcTT] = hcpmpm_reeds_shepp_->TTcTT_path(c1,
                                                                      c2,
                                                                      &cstart[hc_cc_rs::TTcTT],
                                                                      &cend[hc_cc_rs::TTcTT],
                                                                      &qi1[hc_cc_rs::TTcTT],
                                                                      &qi2[hc_cc_rs::TTcTT],
                                                                      &qi3[hc_cc_rs::TTcTT],
        ...
        {
            length[hc_cc_rs::TcTTcT] = hcpmpm_reeds_shepp_->TcTTcT_path(c1,
                                                                        c2,
                                                                        &cstart[hc_cc_rs::TcTTcT],
                                                                        &cend[hc_cc_rs::TcTTcT],
                                                                        &qi1[hc_cc_rs::TcTTcT],
                                                                        &qi2[hc_cc_rs::TcTTcT],
                                                                        &ci1[hc_cc_rs::TcTTcT],
                                                                        &ci2[hc_cc_rs::TcTTcT]);
        ...
        {
            length[hc_cc_rs::TTT] = hcpmpm_reeds_shepp_->TTT_path(c1,
                                                                  c2,
                                                                  &cstart[hc_cc_rs::TTT],
                                                                  &cend[hc_cc_rs::TTT],
                                                                  &qi1[hc_cc_rs::TTT],
        ...
        {
            length[hc_cc_rs::TcST] = hcpmpm_reeds_shepp_->TcST_path(c1,
                                                                    c2,
                                                                    &cstart[hc_cc_rs::TcST],
                                                                    &cend[hc_cc_rs::TcST],
                                                                    &qi1[hc_cc_rs::TcST],
                                                                    &qi2[hc_cc_rs::TcST],
                                                                    &qi3[hc_cc_rs::TcST]);
        ...
        {
            length[hc_cc_rs::TScT] = hcpmpm_reeds_shepp_->TScT_path(c1,
                                                                    c2,
                                                                    &cstart[hc_cc_rs::TScT],
                                                                    &cend[hc_cc_rs::TScT],
                                                                    &qi1[hc_cc_rs::TScT],
                                                                    &qi2[hc_cc_rs::TScT],
                                                                    &qi3[hc_cc_rs::TScT]);
        ...
        {
            length[hc_cc_rs::TcScT] = hcpmpm_reeds_shepp_->TcScT_path(c1,
                                                                      c2,
                                                                      &cstart[hc_cc_rs::TcScT],
                                                                      &cend[hc_cc_rs::TcScT],
                                                                      &qi1[hc_cc_rs::TcScT],
                                                                      &qi2[hc_cc_rs::TcScT]);
        }
    label_end:
        ...
        HC_CC_RS_Path* path;
        path = new HC_CC_RS_Path(c1.start,
                                 c2.start,
                                 best_path,
                                 kappa_,
                                 sigma_,
    ...
    HC_CC_RS_Path* HCpmpm_Reeds_Shepp_State_Space::hcpmpm_reeds_shepp(const State& state1,
                                                                      const State& state2) const
    {
        ...
        // compute the shortest path for the 16 combinations (4 circles at the beginning and 4 at
        // the end)
        HC_CC_RS_Path* path[] = {nullptr,
                                 nullptr,
                                 nullptr,
                                 nullptr,
                                 nullptr,
        ...
        double lg[] = {numeric_limits<double>::max(),
    ...
    double HCpmpm_Reeds_Shepp_State_Space::get_distance(const State& state1,
                                                        const State& state2) const
    {
    ...
    vector<Control> HCpmpm_Reeds_Shepp_State_Space::get_controls(const State& state1,
                                                                 const State& state2) const
    {
