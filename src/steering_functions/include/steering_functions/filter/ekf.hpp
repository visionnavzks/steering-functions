...
namespace steering
{
    ...
    public:
        ...
        void set_parameters(const Motion_Noise&      motion_noise,
        ...
        /** \brief Computes the Jacobians of the motion equations with respect to the state and
         * control */
        void get_motion_jacobi(const State&   state,
        ...
        Matrix2d get_motion_covariance(const State&   state,
                                       const Control& control,
        ...
        /** \brief Predicts the covariances based on the paper:
            Rapidly-exploring random belief trees for motion planning under uncertainty, A. Bry and
           N. Roy, IEEE ICRA 2011 */
        void predict(const State_With_Covariance& state,
                     const Control&               control,
                     double                       integration_step,
                     State_With_Covariance&       state_pred) const;

        /** \brief Predicts the covariances */
        void update(const State_With_Covariance& state_pred,
