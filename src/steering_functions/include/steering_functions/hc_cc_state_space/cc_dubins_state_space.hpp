...
namespace steering
{
    ...
    public:
        /** \brief Constructor */
        CC_Dubins_State_Space(double kappa,
                              double sigma,
                              double discretization = 0.1,
                              bool   forwards       = true);

        /** \brief Predicts a state forwards or backwards to zero and max. curvature */
        std::vector<std::pair<State, Control>> predict_state(const State& state,
                                                             bool         forwards) const;
        ...
        /** \brief Returns all possible control sequences (forward and reverse) from state1 to state2 */
