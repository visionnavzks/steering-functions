...
State integrate_ODE(const State& state, const Control& control, double integration_step)
{
    ...
    state_next.theta = pify(state.theta + state.kappa * d * integration_step +
                            0.5 * sigma * d * integration_step * integration_step);
...
Matrix3d
get_num_motion_jacobi_x(const State& state, const Control& control, double integration_step)
{
    Matrix3d F_x;
...
Matrix32d
get_num_motion_jacobi_u(const State& state, const Control& control, double integration_step)
{
    Matrix32d F_u;
