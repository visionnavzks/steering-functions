...
{
    if (state1.x == state2.x && state1.y == state2.y && state1.theta == state2.theta &&
        state1.kappa == state2.kappa && state1.d == state2.d)
    {
        return true;
    }
    return false;
...
TEST(SteeringFunctions, pathLength)
{
    ...
    for (int i = 0; i < SAMPLES; i++)
    {
        ...
        EXPECT_LT(fabs(cc00_rs_ss.get_distance(start, goal) - get_path_length(cc00_rs_path)),
                  EPS_DISTANCE);

        vector<State> hc_rs_path = hc_rs_ss.get_path(start, goal);
        EXPECT_LT(fabs(hc_rs_ss.get_distance(start, goal) - get_path_length(hc_rs_path)),
                  EPS_DISTANCE);

        vector<State> hc00_rs_path = hc00_rs_ss.get_path(start, goal);
        EXPECT_LT(fabs(hc00_rs_ss.get_distance(start, goal) - get_path_length(hc00_rs_path)),
                  EPS_DISTANCE);

        vector<State> hc0pm_rs_path = hc0pm_rs_ss.get_path(start, goal);
        EXPECT_LT(fabs(hc0pm_rs_ss.get_distance(start, goal) - get_path_length(hc0pm_rs_path)),
                  EPS_DISTANCE);

        vector<State> hcpm0_rs_path = hcpm0_rs_ss.get_path(start, goal);
        EXPECT_LT(fabs(hcpm0_rs_ss.get_distance(start, goal) - get_path_length(hcpm0_rs_path)),
                  EPS_DISTANCE);

        vector<State> hcpmpm_rs_path = hcpmpm_rs_ss.get_path(start, goal);
        EXPECT_LT(fabs(hcpmpm_rs_ss.get_distance(start, goal) - get_path_length(hcpmpm_rs_path)),
                  EPS_DISTANCE);
...
TEST(SteeringFunctions, interpolation)
{
    ...
    for (int i = 0; i < SAMPLES; i++)
    {
        ...
        vector<State> cc_dubins_forwards_path =
            cc_dubins_forwards_ss.integrate(start, cc_dubins_forwards_controls_inter);
        State cc_dubins_forwards_state_inter =
            cc_dubins_forwards_ss.interpolate(start, cc_dubins_forwards_controls, t);
        ...
        EXPECT_EQ(is_equal(cc00_dubins_forwards_path.back(), cc00_dubins_forwards_state_inter),
                  true);
        ...
        EXPECT_EQ(is_equal(cc0pm_dubins_forwards_path.back(), cc0pm_dubins_forwards_state_inter),
                  true);
        ...
        EXPECT_EQ(is_equal(ccpm0_dubins_forwards_path.back(), ccpm0_dubins_forwards_state_inter),
                  true);
        ...
        EXPECT_EQ(is_equal(ccpmpm_dubins_forwards_path.back(), ccpmpm_dubins_forwards_state_inter),
                  true);
