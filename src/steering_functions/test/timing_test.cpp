...
int get_curv_discont(const vector<Control>& controls)
{
    ...
    for (vector<Control>::const_iterator iter2 = next(controls.begin()); iter2 != controls.end();
         ++iter2)
    {
...
void write_to_file(const string& id, const vector<Statistic>& stats)
{
    string path_to_output =
        ros::package::getPath("steering_functions") + "/test/" + id + "_stats.csv";
...
vector<Statistic>
get_controls(const string& id, const vector<State>& starts, const vector<State>& goals)
{
    clock_t           clock_start;
...
vector<Statistic>
get_path(const string& id, const vector<State>& starts, const vector<State>& goals)
{
    clock_t           clock_start;
    clock_t           clock_finish;
    Statistic         stat;
...
TEST(Timing, getControls)
{
    ...
    cout << "[----------] " + cc_dubins_id + " mean [s] +/- std [s]: "
         << get_mean(computation_times) << " +/- " << get_std(computation_times) << endl;
...
TEST(Timing, getPathWithCovariance)
{
    ...
    string            hc_rs_id = "HC_RS";
    vector<Statistic> hc_rs_stats =
        get_path_with_covariance(hc_rs_id, starts_with_curvature, goals_with_curvature);
    computation_times.clear();
    for (const auto& stat : hc_rs_stats)
    {
        computation_times.push_back(stat.computation_time);
    ...
    string            rs_id = "RS";
    vector<Statistic> rs_stats =
        get_path_with_covariance(rs_id, starts_without_curvature, goals_without_curvature);
    computation_times.clear();
    for (const auto& stat : rs_stats)
    {
        computation_times.push_back(stat.computation_time);
